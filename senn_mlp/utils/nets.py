...
from jax.tree_util import tree_reduce as jtr
...
class Passive(nn.Module):
    ...
    def insert_feature(self, feature, index, origin=0):
        ...
        return self.variables.copy(
            {
                "params": jtm(
                    func, self.variables["params"].unfreeze(), feature["params"]
                )
            }
        )
    ...
    def restrict_params(self, in_dim, out_dim):
        ...
        restrict = lambda arr: arr.at[..., out_dim:].set(0.0)
...
class DConv(Passive):
    ...
    def identity_params(self, F):
        params = self.variables["params"]
        return {"Conv_0": {"bias": jnp.zeros(F), "kernel": self.identity_kernel(F)}}
...
class Rational1D(Passive):
    ...
    def __call__(self, x):
        ...
        self.sow("intermediates", "odd", 2.0 * x * den)
    ...
    def null(self):
        null = lambda name: self.variables["params"][name] == 0.0
        return null("w_vec") & null("w_const") & null("w_lin")

    def identity_params(self, F, rank=None):
        params = {"w_vec": jnp.zeros(F), "w_const": jnp.zeros(F), "w_lin": jnp.ones(F)}
        if rank is not None:
            params["w_lin"] = params["w_lin"].at[rank:].set(0.0)
        return params
...
class DDense(nn.Dense, Splittable):
    ...
    def __init__(self, features, init_zero=False):
        if init_zero:
            super().__init__(
                features, kernel_init=initializers.zeros, precision=def_prec
            )
        else:
            super().__init__(
                features, bias_init=initializers.normal(1e0), precision=def_prec
            )
        ...
        return self.variables.copy(
            {
                "params": self.variables["params"].copy(
                    {
                        "kernel": pad_axis(
                            self.variables["params"]["kernel"], count, axis=-2
                        )
                    }
                )
            }
    ...
    def null(self):
        null = (self.variables["params"]["kernel"] == 0.0).all(axis=-2)
        return null & (self.variables["params"]["bias"] == 0.0)
        ...
        return params.copy(
            {
                ...
                "kernel": jnp.zeros_like(old_kernel).at[diag].set(1.0),
            }
        )

    def restrict_params(self, in_dim, out_dim):
        params = super().restrict_params(in_dim, out_dim)
        return params.copy({"kernel": params["kernel"].at[in_dim:, :].set(0.0)})
...
class Layer(Splittable):
    ...
    def setup(self):
        ...
        self.equivariant = (
            None if F is None else [func() for func in self.make_equivariant]
        )
    ...
    def split_params(self, basis, inv_basis, shift):
        ...
        second = self.variables["params"].copy({"linear": second})
        return first, second
    ...
    def restrict_params(self, in_dim, out_dim):
        params = self.variables["params"]
        params = params.copy({"linear": self.linear.restrict_params(in_dim, out_dim)})
        params = params.copy(
            {
...
class Layers(nn.Module):
    ...
    def pad_pow2(self, index, override=False):
        ...
        layer_update(
            index + 1, self.layers[index + 1].pad_inputs(features[index] - old)
        )
    ...
    def split_layer(self, index, basis, shift, inserter):
        ...
        first_linear, second_layer = self.layers[index].split_params(
            basis, shift, kill_in, kill_out
        )
        first_layer = inserter(first_linear)
        params = self.variables["params"]
        for i in reversed(range(index + 1, len(self.layers))):
            params = params.copy({f"layers_{i+1}": params[f"layers_{i}"]})
        params = params.copy(
            {f"layers_{index}": first_layer, f"layers_{index+1}": second_layer}
        )
        return params
    ...
    def activate_layer(self, dims, index, basis, shift):
        ...
        # assert input_dim == 1
        new_rank = (
            sum(~self.layers[preceding].null()) if preceding is not None else input_dim
        )
        new_input_size = (
            self.layers[index - 1].linear.output_size() if index != 0 else input_dim
        )
        basis = basis.at[new_rank:, :].set(0.0)
        basis = basis.at[:, new_rank:].set(0.0)
        shift = shift.at[new_rank:].set(0.0)
        ...
        new_linear, new_succeeding = self.layers[succeeding].split_params(
            basis, inv_basis, shift
        )
                ...
                "params": params.copy(
                    {
                        ...
                        for i, (layer, in_dim, out_dim) in enumerate(
                            zip(self.layers, in_dims, out_dims)
                        )
                    }
                )
            }
    ...
    def restrict_grad(self, dims):
        ...
        for i, (in_dim, out_dim) in enumerate(zip(in_dims, dims)):
            key = f"layers_{i}"
            if out_dim is None:
                params = params.copy({key: jtm(lambda arr: arr * 0.0, params[key])})
            else:
                params = params.copy(
        ...
        # print(dims)
        # print('Done!')
        grad = self.variables.copy({"params": params})
        return grad
...
def batch_ng(func, lossfn, metric, params, x, labels, p, axis=0):
    y, loss, dL_p, d2L_p, dL, Jp, corr = jax.vmap(
        lambda x, l: pass_one(func, partial(lossfn, l), params, p, x), 0
    )(x, labels)
