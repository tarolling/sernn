...
def tree_update(old, direction, scale=1.0):
...
class EMA:
    ...
    def increment_t(self, state):
        return state.copy(
            {"ema": state["ema"].copy({self.tkey: self.get_t(state) + 1})}
        )
    ...
    def variance(self, state):
        return tree_map(
            lambda a, b: a - b, state["ema"][self.sqkey], sqtree(self.mean(state))
        )
    ...
    def update(self, state, value, batch_axis=None):
        ...
        if self.tau is None:
            state = state.copy(
                {"ema": state["ema"].copy({self.key: value, self.sqkey: sqval})}
            )
        else:
            sqtau = self.tau if self.sqtau is None else self.sqtau
            state = state.copy(
                {"ema": state_exp_update(self.key, self.tau, state["ema"], value)}
            )
            state = state.copy(
                {"ema": state_exp_update(self.sqkey, sqtau, state["ema"], sqval)}
            )
...
class Excalibur:
    ...
    def observe(self, model, lossfn, restrict_grad, xs, labels, state):
        ...
        # print('1.2')
        loss, L, L_JP, L2 = vmap(
            lambda l, Y_, JP_: self.beta(partial(lossfn, l), state, Y_, JP_)
        )(labels, Y, JP)
        # print('1.3')
        (fp,), (jl,) = vmap(
            lambda x, JP_, L_: self.gamma(partial(func, x), state, [JP_, L_])
        )(xs, JP, L)
        ...
        state = self.lsqmag.update(
            state, vmap(lambda l: jnp.sum(l**2))(L), batch_axis=0
        )
        # print('2')
        ...
        produ = jnp.sum(
            vectors[:, :, None, ...] * vectors[:, None, :, ...], axis=y_axes
        )
        # print('5')
    ...
    def update_params(self, state):
        recip_tau = 1.0 if self.tau is None else 1 / self.tau
...
class SimpleGradient:
    ...
    def observe(self, func, restrict_grad, xs, labels, params, state):
        maybe_restrict_grad = lambda x: (
            restrict_grad(x) if restrict_grad is not None else x
        )
    ...
    def adam(self, state):
        ...
        return tree_map(lambda a, b: a / (jnp.sqrt(b) + self.epsilon), grad, sqgrad)
...
class KrylovInverter:
    ...
    def observe(self, operator, restrict_grad, xs, tangent, state):
        maybe_restrict_grad = lambda p: (
            restrict_grad(p) if restrict_grad is not None else p
        )
        ...
        interleaved = (
            jnp.zeros((2 * self.order,)).at[0::2].set(evens).at[1::2].set(odds)
        )
    ...
    def inv_pow(self, tangent, exponent, state):
        ...
        indices = (
            jnp.arange(self.order)[:, None] + (jnp.arange(self.order) + 1)[None, :]
        )
        # print(indices)
        # exit()
        ...
        out = jax.vmap(lambda a, b: tree_map(partial(jnp.multiply, a), b))(
            coeffs, shifted_krylov
        )
...
class CGInverter:
    ...
    def __init__(self, cfg, params):
        ...
        assert self.method in [
            "cg",
            "gmres",
        ], f"requested method: {self.method} not recognised"
    ...
    def observe(self, operator, restrict_grad, xs, tangent, state):
        ...
        if self.method == "cg":
            ...
            soln, _ = jax.scipy.sparse.linalg.gmres(
                F, tangent, maxiter=self.order, restart=self.order
            )
...
class KrylovNG:
    ...
    def observe(self, func, lossfn, restrict_grad, xs, ls, params, state):
        ...
        # exit()
        state = self.SG.observe(
            lambda params, x, l: lossfn(l, func(params, x)),
            restrict_grad,
            xs,
            ls,
            params,
        ...
        ID_MUL = 0e1
        sqjacob = lambda tan, x: tree_map(
            lambda a, b: ID_MUL * a + b, tan, bkd_jac(fwd_jac(tan, x), x)
        )
        if self.adam:
...
class CGNG:
    ...
    def observe(self, func, lossfn, restrict_grad, xs, ls, params, state):
        ...
        id_coeff = eps
        tikhonov = lambda tan, x: tree_map(
            jax.lax.add, sqjacob(tan, x), tree_scale(id_coeff, tan)
        )
