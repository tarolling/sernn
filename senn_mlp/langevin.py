...
def tree_normal_like(tree, key) -> PyTreeDef:
    treevec, unflatten = flatten_util.ravel_pytree(tree)
...
def mala_step(lossgradfn, priorvar, old_state, key, lr, temp=1e0, legacy=False):
    ...
    def _delta(prior, state, grad, noise):
        return -lr * (grad / temp + state / prior) + jnp.sqrt(
            2.0 * lr
        ) * noise * jnp.sqrt(prior)
    ...
    _, half_grad = reglossgrad(half_state)
    ...
    accept_prob = jnp.minimum(1.0, jnp.exp(improvement_gap))
